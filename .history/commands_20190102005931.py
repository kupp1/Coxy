...
class Commands():
    ...
    def version(self):
        ...
        latest_ver = (self.recipient, time.time())
    ...
    def time(self):
        ...
        latest_time = (self.recipient, time.time())
        ...
        if ctcp_ans and kirc.Parse(msg).command == 'NOTICE':
            ...
            if ctcp == 'VERSION':
                if latest_ver and time.time() - latest_ver[1] < 300:
                    self.irc.send_privmsg(latest_ver[0], text)
            elif ctcp == 'TIME':
                if latest_time and time.time() - latest_time[1] < 300:
                    self.irc.send_privmsg(latest_time[0], text)
                latest_time = None
