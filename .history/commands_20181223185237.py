...
class Commands():
    ...
    def write(self, groups: list):


    def test(self, groups: list):
        """Тестовая команда"""
        self.irc.send_privmsg(self.recipient, 'It Works!')

    def uptime(self, groups: list):
        """Аптайм бота"""
...
class CommandsCore():
    ...
    def __init__(self, irc: kirc.Irc, prefix: str):
        ...
        for command in self.commands:
            ...
            try:  # пробуем запросить параметр команды
                self.commands_info[command]['pub_use'] = eval(
                    'Commands.%s.pub_use' % command)
            ...
            try:
                self.commands_info[command]['priv_use'] = eval(
                    'Commands.%s.priv_use' % command)
            except AttributeError:
                self.commands_info[command]['priv_use'] = self.default_priv_use
            try:
                self.commands_info[command]['required_args'] = eval(
                    'Commands.%s.required_args' % command)
            except AttributeError:
                self.commands_info[command]['required_args'] = self.default_required_args
            try:
                self.commands_info[command]['pub_cooldown'] = eval(
                    'Commands.%s.pub_cooldown' % command)
            except AttributeError:
                self.commands_info[command]['pub_cooldown'] = self.default_pub_cooldown
            try:
                self.commands_info[command]['priv_cooldown'] = eval(
                    'Commands.%s.priv_cooldown' % command)
            except AttributeError:
    ...
    def args_check(self, command: str, msg: str):
        ...
        if len(self.args) >= required_args:
            ...
            self.irc.send_notice(self.irc.get_nick(
                msg), 'required_args: %s' % required_args)
            return False
