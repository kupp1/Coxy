...
RFC2812 = re.compile(r""" # Оригинальная ругулярка взята с https://gist.github.com/datagrok/380449c30fd0c5cf2f30
...
class Parse:
    ...
    Позволяет более красивым образом запрашивать группу по «назвнию»:
        Parse(строчка).host напрмиер возвращает хост.
    """
...
class Irc():
    ...
    def __init__(self, name: str, host: str, port: int, nick: str, username: str, realname: str, encoding:str,
        ...
        self.ping_match = re.compile('^PING :.*')  # регулярка дли пингов сервера
    ...
    def connection_alive(self):
        ...
        self.__sock.settimeout(1) # устанавливаем одну секунду как минимальное время пинга до сервера
        try:
            msg = self.__sock.recv(4096).decode(self._encoding, 'ignore')  # ждем ответа
        except socket.timeout:
            self.__sock.settimeout(self._ping_timeout)  # возвращаем иходный таймаут
            return False  # вердикт: конект оборвался
        else:  # если тймаута все-таки не произошло
        ...
        while attempt <= connect_attempts and not connected:  # пока попытки не исчерпаны и коннекта нет
            ...
            try:
                try:  # пробуем запросить ip
                    ...
                    raise IrcConnectionError('Name or service not known: %s' % self._host)
                ...
                pretty_print('connected to %s:%s' % (self._host, str(self._port)))  # файтически коннект есть
                self.send('NICK %s' % self._nick)  # отправляем свои данные согласно rfc
                self.send('USER %s 0 * :%s' % (self._username, self._realname))
                connect_time = time.time()
                self.__sock.settimeout(self._ping_timeout + 1)
                while True:  # ждем от сервера приветственнго сообщения
                    if time.time() - connect_time > connect_timeout:
                        raise IrcConnectionError('Irc server timeout')  # если сообщение не пришло, то работать не можем
                    msgs = self.wait_data()
                    if msgs:
                        for msg in msgs.split('\r\n'):
                            if msg:
                                ...
                                if Parse(msg).command == '001': # если пришло
                                    pretty_print('Connection established') # то пожалуй все хорошо
                                    connected = True
                    if connected:
                        break
    ...
    def is_privmsg(self, msg: str, private=None):
        """
        Проверка является ли msg соосбщением типа privmsg.
    ...
    @staticmethod
    def is_action(msg: str):
        return Parse(msg).command == 'PRIVMSG' and Parse(msg).content.find('\x01ACTION') == 0
    ...
    def _privmsg_cut(self, recipient: str, text: str, start: int, end: int):
        """Разделяет блинное сообщение на части, т.к. в irc privmsg не может веить больше 512 байт (514: два на \r\n)"""
        if sys.getsizeof(('PRIVMSG %s :%s\r\n' % (recipient, text[start:end])).encode()) <= 514:
            self.send('PRIVMSG %s :%s' % (recipient, text[start:end]))
            pretty_print('Send privmsg to %s :%s' % (recipient, text[start:end].strip()))
        else:
            ...
            if sys.getsizeof(('PRIVMSG %s :%s\r\n' % (recipient, text[end:len(text)])).encode()) > 514:
                ...
                pretty_print('Send privmsg to %s :%s' % (recipient, text[end:len(text)].strip()))
