...
def collect_stat(irc: kirc.Irc, msg: str):
    msg_content = kirc.Parse(msg).content
    if irc.is_privmsg(msg):
        try:
...
class Commands():
    ...
    def snow(self):
        global cutlet_
        target = self.args[0] or self.nick
        self.irc.send_action(self.recipient, 'Кинул снежком в %s и %s' % (
            target, random.choice(cutlet_)))
