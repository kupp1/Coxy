    ...
    def search(self, msg: str):
        ...
        if ctcp_ans and kirc.Parse(msg).command == 'NOTICE':
            ...
            if ctcp == 'VERSION':
                ...
                print(bool(latest_ping))
                print(bool(str(latest_ping[1]) == text))
                print(bool(time.time() - latest_ping[1] < 500))
                if latest_ping and str(latest_ping[1]) == text and time.time() - latest_ping[1] < 500:
                    self.irc.send_privmsg(latest_ping[0], '%s PING: %.3f' % (
                        irc_format.IrcTextFormat(latest_time[2]).bold, time.time() - float(text)))
                latest_ping = None
