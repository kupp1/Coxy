    ...
    def help_command(self):
        if self.args[0]:
            try:
                ...
                self.irc.send_notice(self.nick, 'No such command %s' % self.args[0])
            else:
                command = self.args[0]
                self.irc.send_notice(self.nick, 'description: %s' % eval('self.%s.__doc__' % command)) 
                for parameter in self.commands_info[command]:
                    self.irc.send_notice(self.nick, '%s: %s' % (parameter, self.commands_info[command][parameter]))
        else:
            self.irc.send_notice(self.nick, 'I have several commands: %s' % ', '.join(list(self.commands_info.keys())))
    help_command.regexp = 'help(?: +(.+))?'
...
class CommandsCore():
    ...
    def search(self, msg: str):
        ...
        if ctcp_ans and kirc.Parse(msg).command == 'NOTICE':
            ...
            text = grps[1]
            if ctcp == 'VERSION':
                if latest_ver and time.time() - latest_ver[1] < 300:
                    self.irc.send_privmsg(latest_ver[0], '%s VERSION: %s' % (irc_format.IrcTextFormat(latest_ver[2]), text))
                latest_ver = None
            elif ctcp == 'TIME':
                if latest_time and time.time() - latest_time[1] < 300:
                    self.irc.send_privmsg(latest_ver[0], '%s TIME: %s' % (irc_format.IrcTextFormat(latest_ver[2]), text))
                latest_time = None
