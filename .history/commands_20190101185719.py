    ...
    def help_command(self):
        ...
        if self.args[0]:
            try:
                ...
                command = self.args[0]
                self.irc.send_notice(self.nick, 'description: %s' % eval('self.%s.__doc__' % self.args[0])) 
                for parameter in self.commands_info[self.args[0]]:
                    self.irc.send_notice(self.nick, '%s: %s' % (parameter, self.commands_info[self.args[0]][parameter]))
        else:
