        ...
        for command in self.commands:
            ...
            for parameter in parameters:
                try:  # пробуем запросить параметр команды
                    self.commands_info[command][parameter] = getattr(Commands, '%s.%s' % (command, parameter))
            ...
            try:
                self.commands_info[command]['regexp'] = re.compile('^%s%s' (self.prefix, getattr(Commands, '%s.regexp' % command)))
            except AttributeError:
                self.commands_info[command]['regexp'] = re.compile('^%s%s' % (self.prefix, command))
