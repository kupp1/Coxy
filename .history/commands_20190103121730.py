...
class Commands():
    ...
    def badum_ts(self):
        irc.send_privmsg(recipient, 'https://goo.gl/D7SQSc')

    def shrug(self):
        global shrugs
        irc.send_privmsg(self.recipient, random.choice(shrugs))
    ...
    def tg(self):
        irc.send_privmsg(self.recipient, 'https://t.me/sixteen_bits')

    def steam(self):
        irc.send_privmsg(self.recipient, 'https://steamcommunity.com/groups/BochaClub')

    def udp(irc: IrcClient, recipient: str, args: list, nick):
        global udp_
        target = self.args[0] or self.nick
        self.irc.send_action(self.recipient, 'Кинул udp-пакетом в %s… %s' % (args[0], random.choice(udps)))
