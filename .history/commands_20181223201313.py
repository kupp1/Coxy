...
class Commands():
    ...
    def write(self):
        string = self.args[0]
        if string:
            self.irc.send_privmsg(self.recipient, self.args[0])
    write.regexp = 'write (.+)'
        ...
        for command in self.commands:
            ...
            for parameter in parameters:
                try:  # пробуем запросить параметр команды
                    self.commands_info[command][parameter] = eval('Commands.%s.%s' % (command, parameter))
            ...
            try:
                self.commands_info[command]['regexp'] = re.compile('^%s%s' % (self.prefix, eval('Commands.%s.regexp' % command)))
