...
class Commands():
    ...
    следующими входными данными:
        irc - ссылка на основной класс, из коорого производится работа с irc (описан в kirc.py),
        recipient - куда следует отправить команду, может быть ником или каналом,
        args - аргументы пользователя к команде,
        nick - ник вызвавщего команду.
    Каждая команда имеет параметры:
        ...
        required_args: минимальное количество аргументов к команде (целове число >= 0)
        pub_cooldown: задержка для использования команды на каналах для каждого пользователя (в секундах >=0)
        priv_cooldown: задержка на использование в привате для конктретного пользователя (в секундах >= 0)
    Все параметры команд для команд статичны и не могут быть изменены в хоже исполнения.
    ...
    def uptime(self):
        ...
        if hours > 24:
            days = int(hours // 24)
            hours = int(hours % 24)
        self.irc.send_privmsg(self.recipient, 'Bot uptime: %d days, %d hours, %d minutes' % (days, hours, minutes))


class CommandsCore():
    """
    Данный класс требует, в отличии от Commands (выше) создания экземпляра.
    Основная идея: в теле бота создается экзепляр данного класса. Далее в основной цикл вставляется лишь одна строчка:
    ...
    которые описаны в классе Commands и анализируютя специфические параметры команд (описаны в Commands).
    ...
    несколько этапов:
        1. Поиск запроса команды (метод call_search). За специфическое сочетание-запрос, который ищется только в начале
           ...
           команды префикс (указывается как параметр при создании экзепляра класса CommandsCore).
        ...
        3. Проверка может ли данный пользователь вызвать команду (метод cooldown_check). Каждая команда имеет кулдаун
           (он же делей) или не имеет, если он равен 0. Кулдаун считается для каждого пользоватедя на каждом канале и в
        ...
        4. Проверка введено ли пользователем необходимое количество аргументов (метод args_check) для данной команды.
           Правильность аргументов на данном этапе не проверяетяся, этим занимаются сами методы команд, описанные в
           классе Commands.
    Все проверяется сверху вниз. Если что-то не соотвествует, то проверка прерывается и метод команды не вызывается.
    Все нюансы этапов описаны в методах ниже.
    """
    def __init__(self, irc: kirc.Irc, prefix: str):
        r = re.compile('__.*__')  # специфическая регулярка для отбора всех методов команд из класса Commands
        self.irc = irc
        self.commands = [method for method in dir(Commands) if not r.search(method)]
        """
        В строчке выше быстрым созданием списка отбираются все методы из класса Commands, которые не соотвествуют
        регулярке __.*__ (в таком формате называются все зарезирвированные-встроенные методы классов, смотрите
        специфику python3).
        В список commands отбираются только имена методов. Далее они используюется как имена команд.
        """
        self.prefix = re.escape(prefix)

        """
        Ниже заданы стандартные значения параметров для команд.
        Значеня параметров описаны в Commands.
        """
        self.default_pub_use = True
        self.default_priv_use = True
        self.default_required_args = 0
        self.default_pub_cooldown = 120
        self.default_priv_cooldown = 0

        self.commands_info = {}  # cоздаём пустой словарь для параметров
        for command in self.commands:
            self.commands_info[command] = {}
            try:  # пробуем запросить параметр команды
                self.commands_info[command]['pub_use'] = eval('Commands.%s.pub_use' % command)
            except AttributeError:  # если его нет, то присваеваем дефолное значение
                self.commands_info[command]['pub_use'] = self.default_pub_use
            try:
                self.commands_info[command]['priv_use'] = eval('Commands.%s.priv_use' % command)
            except AttributeError:
                self.commands_info[command]['priv_use'] = self.default_priv_use
            try:
                self.commands_info[command]['required_args'] = eval('Commands.%s.required_args' % command)
            except AttributeError:
                self.commands_info[command]['required_args'] = self.default_required_args
            try:
                self.commands_info[command]['pub_cooldown'] = eval('Commands.%s.pub_cooldown' % command)
            except AttributeError:
                self.commands_info[command]['pub_cooldown'] = self.default_pub_cooldown
            try:
                self.commands_info[command]['priv_cooldown'] = eval('Commands.%s.priv_cooldown' % command)
            except AttributeError:
                self.commands_info[command]['priv_cooldown'] = self.default_priv_cooldown
            """
            В коде выше используется eval. Это зло, но что-то подобное в данном случае для анализа статических переменных
            другого класса необходимо. Отмечу, что в __dict__ статические переменные не обображаются. Именно сатические
            переменные выбраны в Commands для ввода комманд только из-за стилистики кода.
            """

            self.commands_info[command]['pub_cooldown_string'] = self.delay2str(self.commands_info[command]['pub_cooldown'])
            self.commands_info[command]['priv_cooldown_string'] = self.delay2str(self.commands_info[command]['priv_cooldown'])
    ...
    def right_check(self, command: str):
        if self.is_private:
            if self.commands_info[command]['priv_use']:
                ...
                self.irc.send_privmsg(self.nick, '%s only for public' % command)
                return False
        else:
            if self.commands_info[command]['pub_use']:
                return True
            else:
                self.irc.send_privmsg(self.nick, '%s only for private' % command)
                return False

    @staticmethod
    def delay2str(delay):
        """convert seconds to pretty string with hours, days and seconds, for dalay notice"""
        seconds = int(delay)
        minutes = 0
        hours = 0
        days = 0
    ...
    def cooldown(cooldowns: dict, timer: int, value: str):
        if timer != 0:  # если кулдаун 0, то и проверять нечего
            if value in cooldowns:  # если запись найдена
                if time.time() - cooldowns[value] >= timer:  # если срок кулдауна истек
    ...
    def cooldown_check(self, command: str):
        ...
        Кулдаун записывается следующим образом:
            1. В качестве ключа выступает специфическая запись, которая характеризует к чему она относится.
               ...
               запись дублируется. Пусть хранятся лишнией символ ключах, а код остается понятнее.
            2. В качестве значения выступает время послднего успешного использовния пользователем команды. Т.е. если
    ...
    def args_check(self, command: str, msg: str):
        self.args = self.content.split()[1:]
        required_args = self.commands_info[command]['required_args']
        if len(self.args) >= required_args:
            return True
        else:
            self.irc.send_notice(self.irc.get_nick(msg), 'required_args: %s' % required_args)
            return False
    ...
    def call_search(self, msg: str):
        ...
        for command_i in self.commands:
            if re.search('^%s%s' % (self.prefix, re.escape(command_i)), self.content, re.IGNORECASE):
                command = command_i
        return command
        ...
        if command:
            if self.right_check(command):
                if self.cooldown_check(command):
                    if self.args_check(command, msg):
                        eval('Commands(self.irc, self.recipient, self.args, self.nick).%s()\n' % command)
