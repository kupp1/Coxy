...
irc_nick_match = re.compile('^\A[a-zа-я_\-\[\]\\^{}|`][a-zа-я0-9_\-\[\]\\^{}|`]{2,15}$', re.IGNORECASE | re.UNICODE)
domain_match = re.compile('^(?!:\/\/)([a-zA-Z0-9-_]+\.)*[a-zA-Z0-9][a-zA-Z0-9-_]+\.[a-zA-Z]{2,11}?$', re.IGNORECASE)
...
class Commands():
    ...
    def ip(self):
        ...
        if is_url:
            ...
            return
        api = 'http://api.ipstack.com/%s?access_key=%s&format=1' % (ip, ipstack_token)
    ...
    def kitty(self):
        ...
        global kitty_
        target = self.args[0] or self.nick
        self.irc.send_action(self.recipient, 'кинул кошечку в %s, та %s' % (target, random.choice(kitty_)))
    ...
    def slap(self):
        ...
        self.irc.send_action(self.recipient, 'slaps %s around a bit with a %s' % (target, thing))
    ...
    def cutlet(self):
        global cutlet_
        target = self.args[0] or self.nick
        self.irc.send_action(self.recipient, 'Кинул котлетой в %s и %s' % (target, random.choice(cutlet_)))
    ...
    def hi(self):
        global cutlet_
        target = self.args[0] or self.nick
        self.irc.send_action(self.recipient, 'Кинул приветом в %s и %s' % (target, random.choice(cutlet_)))
    ...
    def snow(self):
        global cutlet_
        target = self.args[0] or self.nick
        self.irc.send_action(self.recipient, 'Кинул снежком в %s и %s' % (target, random.choice(cutlet_)))
    snow.regexp = '(?:снежок|snowball)(?:\s+(.+)?)?'
    snow.priv_use = False
    ...
    def steam(self):
        self.irc.send_privmsg(self.recipient, 'https://steamcommunity.com/groups/BochaClub')

    def udp(self):
        global udp_
        target = self.args[0] or self.nick
        self.irc.send_action(self.recipient, 'Кинул udp-пакетом в %s… %s' % (self.args[0], random.choice(udp_)))
    udp.regexp = 'udp(?:\s+(.+)?)?'
    ...
    def stat(self):
        ...
        try:
            ...
            irc.send_privmsg(recipient, 'No such user')
        else:
            try:
                ...
                wpm = 0
            irc.send_privmsg(
                recipient, '\x02%s\x02 send %d words, %d messages, %d symbols (\x02%.2f\x02 wpm) and join %d times' %
                        (target, row.words_count, row.msg_count, row.symbols_count, wpm, row.join_count)
                            )
